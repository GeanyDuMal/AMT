...
function uploadFileToFtp($ftp_conn, $filePattern): void
{
    foreach (glob(createPathFromRootProject($filePattern)) as $filename) {
        if (ftp_put($ftp_conn,
            str_replace(absolutePathProjectRoot, "", $filename),
            $filename,
            FTP_ASCII))
        {
            echo "Successfully uploaded $filename at the location ".str_replace(absolutePathProjectRoot, "", $filename)."
            ";
        } else {
            echo "Error uploading $filename at the location ".str_replace(absolutePathProjectRoot, "", $filename)."
            ";
        }
    }
}